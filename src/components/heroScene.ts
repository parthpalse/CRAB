...
export function initHeroScene(canvas: HTMLCanvasElement, onReveal: () => void) {
  ...
  const nodesMat = new THREE.PointsMaterial({ color:0xe6e6e6, size:0.10, map:nodeTex, transparent:true, depthWrite:false, blending:THREE.AdditiveBlending, opacity:0.95 });
  ...
  const eMat = new THREE.LineBasicMaterial({ color:0x8a8a8a, transparent:true, opacity:0, depthWrite:false, blending:THREE.AdditiveBlending });
  ...
  const sigMat = new THREE.MeshBasicMaterial({ color:0xffffff, transparent:true, opacity:0, blending:THREE.AdditiveBlending, depthWrite:false });
  ...
  const coreShellWire = new THREE.LineSegments(new THREE.WireframeGeometry(wg), new THREE.LineBasicMaterial({ color:0x8a8a8a, transparent:true, opacity:0, blending:THREE.AdditiveBlending, depthWrite:false }));
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const rm = new THREE.LineBasicMaterial({ color:0x8a8a8a, transparent:true, opacity:0, blending:THREE.AdditiveBlending, depthWrite:false });
  ...
  const particlesPoints = new THREE.Points(pGeo, new THREE.PointsMaterial({ color:0xe6e6e6, size:0.018, transparent:true, opacity:0.55, blending:THREE.AdditiveBlending, depthWrite:false }));
  ...
  sgrd.addColorStop(0,'#bbbbbb'); sgrd.addColorStop(0.3,'#bbbbbb88'); sgrd.addColorStop(1,'#bbbbbb00');
  sctx.fillStyle=sgrd; sctx.fillRect(0,0,256,256);
  const glowMat = new THREE.SpriteMaterial({ map:new THREE.CanvasTexture(sc), transparent:true, blending:THREE.AdditiveBlending, depthWrite:false, opacity:0 });
  ...
  function animate() {
    ...
    eMat.opacity=edgeOp;
    if (edgeOp>0.05) {
      for (let i=0;i<edgePairs.length;i++) { const [a,b]=edgePairs[i]; const na=neurons[a].current,nb=neurons[b].current; ePos[i*6]=na.x;ePos[i*6+1]=na.y;ePos[i*6+2]=na.z;ePos[i*6+3]=nb.x;ePos[i*6+4]=nb.y;ePos[i*6+5]=nb.z; }
